...
## Failed Breakout（突破失敗反轉）

Brooks 框架中明確定義的高確定性反轉 setup。

**成立條件（缺一不可）：**
1. 有一根夠格的突破棒——大實體、收近極端、通常是當時段的最大棒之一
2. 價格突破後無法持續，反向穿回突破點以下（或以上）

**為何高確定性：**
追突破的多方（空方）被困住，停損單觸發後成為反向的賣壓（買壓），形成「追損加速」效應。

**與 BOM 測試失敗的區別：**

| 情境 | 突破棒強度 | 失敗的意義 |
|---|---|---|
| Failed Breakout | 強棒（大實體、收近極端） | 多方被套，直接觸發反向進場 |
| BOM 測試失敗 | 弱棒或中等棒（只是探出邊界） | 仍在 TR 中，等待另一側確認 |

**判斷步驟：**
```
突破棒是強棒？
├── 是 → 若隨後反向穿回 → Failed Breakout，signal bar 確認後直接進反向
└── 否 → 只是測試失敗，BOM 繼續，不構成 Failed Breakout 進場依據
```

**重要**：「有沒有 Failed Breakout」的判斷必須先確認突破棒本身的強度，不能只看後來的反轉。
